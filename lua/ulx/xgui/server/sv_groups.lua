...
function groups.init()
	ULib.ucl.registerAccess( "xgui_managegroups", "superadmin", "允许在XGUI中通过用户组标签管理用户组、用户和权限字符串。", "XGUI" )
		...
		if ULib.ucl.query( ply, "ulx addgroup" ) then
			...
			while group do
				if group == args[1] or args[1] == args[2] then
					ULib.clientRPC( ply, "Derma_Message", "无法设置继承！你不能从你正在设置继承的组继承权限", "XGUI提示" )
					return
				end
	...
	function groups.refreshTeams()
		...
		if #emptyteams > 0 then
			local output = "//这个文件存储没有分配用户组的队伍（因为ULX会删除它们）。不要编辑此文件！\n"
